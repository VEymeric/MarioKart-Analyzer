...
def level_name(video, frame_to_check, score_level):
    if video.count == frame_to_check:
        score, name, list_score, list_name = score_compare_image_and_folder("../ressources/maps/", video.gray_frame[:100])
        if score < score_level:
            video.logger.info("%s : %s", str(name), str(score))
            video.gp.run.append(Run(video.gp.nb_player, name, video.count))
            video.gp.state += 1
            #cv2.imwrite("../ressources/TEST/" + str(video.count) + "_level.jpg", video.frame)  # save frame as JPEG file
...
def selection_perso(video):
    ...
    if (abs(video.frame[yT][xL][0] - zJ1[0]) < 20 and abs(video.frame[yT][xL][1] - zJ1[1]) < 20 and abs(video.frame[yT][xL][2] - zJ1[2]) < 20) and \
            (abs(video.frame[y_seccure_2][x_secure_2][0] - zJ1_2[0]) < 20 and abs(video.frame[y_seccure_2][x_secure_2][1] - zJ1_2[1]) < 20 and abs(video.frame[y_seccure_2][x_secure_2][2] - zJ1_2[2]) < 20):
...
def grand_prix(video):
    ...
    if ((abs(bleu - kJ1[0]) < 10 and abs(vert - kJ1[1]) < 30 and abs(rouge - kJ1[2]) < 10) or (abs(bleu - kJ2[0]) < 10 and abs(vert - kJ2[1]) < 20 and abs(rouge - kJ2[2]) < 20)) and ((abs(blue - kJ1[0]) < 10 and abs(green - kJ1[1]) < 35 and abs(red - kJ1[2]) < 30) or (abs(blue - kJ3[0]) < 10 and abs(green - kJ3[1]) < 15 and abs(red - kJ3[2]) < 10)):
...
if __name__ == "__main__":
    screen = cv2.imread("../TEST/17400.jpg")
    x1 = 490
    y1 = 110
    y2 = 970
    kJ1 = [16, 185, 40]  # Vert
    kJ2 = [15, 114, 243]  # Orange
    kJ3 = [247, 123, 0]  # Bleu
    (bleu, vert, rouge) = screen[y1, x1]
    print (bleu, vert, rouge)
    (blue, green, red) = screen[y2, x1]
    print (blue, green, red)

    if ((abs(bleu - kJ1[0]) < 10 and abs(vert - kJ1[1]) < 30 and abs(rouge - kJ1[2]) < 10) or (
                abs(bleu - kJ2[0]) < 10 and abs(vert - kJ2[1]) < 20 and abs(rouge - kJ2[2]) < 20)) and ((
                abs(blue - kJ1[0]) < 10 and abs(green - kJ1[1]) < 35 and abs(red - kJ1[2]) < 30) or (
                abs(blue - kJ3[0]) < 10 and abs(green - kJ3[1]) < 15 and abs(red - kJ3[2]) < 10)):
        print("Classement du GrandPrix")
        print(True)
    print(False)